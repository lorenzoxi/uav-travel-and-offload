...
=======
    ...
    auto points = generate_roi_grid(1000,
                                    100); // each point has _id, lat, lon, z (z unused here)

    vector<int> C = {1, 2, 3, 4, 5, 6, 7, 8};
    std::vector<SpreadingFactor> F = SF125;

    // (example sensors, unused in this snippet)
    const std::vector<Sensor> S = {
            Sensor(0, 0.0, 0.0, 0.0, 30),
            Sensor(1, 0.01, 0.01, 0.0, 30),
            Sensor(2, 0.02, 0.02, 0.0, 30),
            Sensor(3, 0.03, 0.03, 0.0, 30),
            Sensor(4, 0.04, 0.04, 0.0, 30),
            Sensor(5, 0.05, 0.05, 0.0, 30),
            Sensor(6, 0.06, 0.06, 0.0, 30),
            Sensor(7, 0.07, 0.07, 0.0, 30),
            Sensor(8, 0.08, 0.08, 0.0, 30),
            Sensor(9, 0.09, 0.09, 0.0, 30),
            Sensor(10, 0.1, 0.1, 0.0, 30)
    };
    ...
    // start / end (ensure they exist in points)
    int start_point = 1;
    int end_point = 100;
    int H_cost = 80;
    ...
    for (int t = 0; t <= T; ++t) {
        model.addConstr(h[{t}] <= 1e6 * (1.0 - y[{t, end_point}]),
    ...
    for (int t = 0; t < T; ++t) {
        for (auto &i: points)
            for (auto &j: points) {
                double d = ::euc_distance(i, j);  // ensure this returns 0 if i._id == j._id
                cost_movement += d * x[{t, i._id, j._id}];
            }
    }
    GRBLinExpr cost_hoovering = 0;
    for (int t = 0; t <= T; ++t) {
